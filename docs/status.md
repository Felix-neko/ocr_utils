...
| external OCR: experimental (после v16) | `ocr_utils/experimental` | исследование | склейка разорванных переносов по словарю — порог пройден (`pymorphy3`, правило E: 77/79 переносов, 36/36 на с. 47, 1/234 ложных) — кандидат на перенос в `structure`; закрыты: детектор корешка как источник подсказок (не различает «скрыто»/«видно»), короткий промпт (теги те же, структура теряется), полосы 1×N вместо сетки (CER тот же, цена +15…+58 %) | `reports/external_ocr_damaged_research.md`, `ocr_utils/experimental/README.md` |
| geometry_regression (FineReader ухудшил геометрию) | `research/geometry_regression` | исследование | попарные метрики «с коррекцией − без»: поле смещений (тайлы без пары в line art), LSD-штрихи (мм ухода конца, параллельность), строки (наклон, волна, растяжение по тем же глифам), кромки; вердикт bad/mixed/ok с относительным гистерезисом «порча против выигрыша» (v8): эталон 59 из 63 (v7 — 54), прогон по паку — `pack1_v8`; DeepSeek порчи не видит | `reports/geometry_regression.md` |
...
- Внешний OCR, experimental (`reports/external_ocr_damaged_research.md`): перенести склейку переносов (`pymorphy3`, правило E) в `external_ocr_services.structure` — решение пользователя; ловить кодом режим «`<gap>` вместо `<supplied>`» (`gap ≥ 20` при `supplied ≤ 1` → повтор в следующем режиме JSON); решить судьбу `<unclear>`.
