...
- Заполнитель пропуска `<gap>▒…</gap>` без потолка длины — DeepSeek при `temperature 0` зациклился на «▒» до потолка 16k токенов (повреждённая полоса IMG_0008_L, 15 тыс. символов); в промпте «не больше шести», в разборе ряд режется, ответ с рядом от 20 «▒» уходит на повтор в следующем режиме JSON (`ocr_utils/external_ocr_services/schema.py`).
