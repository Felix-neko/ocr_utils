...
| final_pdfs (финальные PDF) | `ocr_utils/final_pdfs` | рабочее | постранично из двух прогонов FineReader: растр в базе или вердикт `bad` детектора геометрии → без коррекции, иначе с коррекцией; правка слоя `text_layer_fix` + surya; иллюстрации JPEG 300 dpi q75 (серые одноканальные, σ 0.05 мм против растровой сетки) из очищенных полос `blurred/`; образы-фигуры FineReader под иллюстрациями снимаются (он кладёт фото отдельными JBIG2, под ними пусто); привязка страница ↔ полоса по порядку со сверкой размера образа; сверка каждой страницы по сохранённому файлу | `reports/final_pdfs_pack1.md` |
...
| final_pdfs: иллюстрации и поля | 300 dpi, JPEG 75, Gaussian σ 0.05 мм + INTER_AREA; поля промежуточной PDF 12.192 / 6.096 мм (288 / 144 px при 600 dpi); образ FineReader снимается при ≥ 80 % площади внутри иллюстрации; полностраничный растр — ≥ 97 % полосы | `ocr_utils/final_pdfs/{pictures,cli}.py`, `run_scripts/final_pdfs/run_pack1.sh` | проба на 4 врезках (2026-09-19), замер образов no-geo PDF |
