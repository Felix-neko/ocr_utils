...
| text_layer_fix: вердикты и чтение | DELETE при перекрытии рамки слова зоной ≥ 0.6, SANITIZE 0.15–0.6 (усечение по глифам, порог глифа 0.5), SUSPECT — россыпь в 4 мм; чтение принято при уверенности ≥ 0.6 и слове от 3 букв, число без букв — ≥ 0.9; прямое чтение снимает зону при +0.1 к уверенности и ≥ 2 буквах; Docstrum: глиф 0.8–7 мм, сосед ≤ 1.15 высоты, цепочка от 3 глифов | `research/text_layer_fix/{classify,ocr,docstrum}.py` | `reports/text_layer_fix.md` |
...
- Текстовый слой финальных PDF (`research/text_layer_fix`): встроить стадию в `pdf_utils` после сборки финальных PDF (run по паку ~1 ч, second-opinion, fix); принимать числа без букв на схемах от 0.8 без ведущих нулей; смешанные ячейки без найденной боковой части (195 из 217) — делить по смене оси; рукописный курсив подписей на чертежах tesseract не берёт — оценить долю, добираемую surya (`reports/text_layer_fix.md`).
