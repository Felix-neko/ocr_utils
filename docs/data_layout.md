...
`run_scripts/scan_markup/pack1/common.sh` (пак-1) и `run_scripts/scan_markup/pack2/common.sh` (пак-2) —
при расхождении правы они, а не этот файл.
...
| `/mnt/system` | Том с данными, SSD | **Регистр значим:** с 2026-09-20 том смонтирован как /mnt/system строчными; прежний /mnt/SYSTEM заглавными больше не существует. Старое написание осталось в отчётах и логах; хук такой путь блокирует |
| `~/Projects/mts_markup` | Рабочие файлы разметки пака-1 (плоско в корне) и пака-2 (`pack2/`), SSD | Базы, превью для CVAT, отладочные оверлеи. Базы невоспроизводимы (ручная разметка неделями) — `common.sh` бэкапит их перед каждым шагом |
...
| `LAYOUT_CACHE_DIR` | `/mnt/system/raw/mts/pack1_page_layout` | **Кэш surya layout** (`page_layout.surya.SuryaCache`): `<вариант>/<полоса>.json`. `scan/` и `sharpened/` перенесены из старых pickle (2026-09-21, `legacy`, полны на 12 135 полос); `fr_nogeo/`, `fr_geo/` — страницы PDF FineReader (`<stem>/pNNNN.json`), набивает `page_layout prefill-surya` (~0.7 с GPU на страницу). `detect --layout-cache` читает и пополняет | SYSTEM |
| — | `/mnt/system/raw/mts/pack1_table_research/layout_surya_готовое`, `layout_surya` | Старые pickle-кэши (с сырым ответом surya); конвейер их больше не читает, стенд таблиц читает при `--layout-dir` на них | SYSTEM |
...
| `EXTERNAL_OCR_SERVICES_ROOT` | `/mnt/system/raw/mts/pack1_external_ocr_services` | Боевой внешний OCR. `out/` — только конечные md выпусков `out/{год}/{год}_{выпуск}.md`, больше туда ничего не пишется. Всё рабочее — в `pages/` (с 21.09.2026; раньше лежало в `out/`): `pages/{год}/{выпуск}/полоса.{md,json,meta.json}` + `toc.json`/`toc.md` на выпуск, рядом `pages/{год}/{год}_{выпуск}.pages.json` (привязка md выпуска к полосам, итог сверки с оглавлением), в корне `summary.csv`, `run.log`. `debug/` с сырыми ответами, промптами и тайлами, `cache/` — кэш запросов (папка на каждый запрос к модели по всем проходам; не удалять — это «продолжить с места»), `probe/` для проб. `missed_toc.txt` в `pages/` — оглавления, найденные моделью вне базы | SYSTEM |
...
Тот же журнал, 1977, выпуски 01–06: один годовой комплект, 597 полос, TIFF RGB по 19–35 МБ,
раскладка `1977/{выпуск}/NNNN_{1L,2R}.tif` (без префикса `IMG_`; в каждом выпуске ещё `77_NN.ScanTailor`).
**«(300 dpi)» в имени папки — про исходный скан, файлы апскейлены до 600 dpi**: тег TIFF 600,
кадры ~3350×5850 px; замер набора (2026-09-21, 4 полосы) — шаг строк 87–97 px, штрих 6–8 px, то же,
что у пака-1. Поэтому все пиксельные параметры run-скриптов и делитель CVAT (8) взяты у пака-1 без
пересчёта. Скрипты — `run_scripts/scan_markup/pack2/` (шаги 1, 2, 3, 7 с той же нумерацией, что у пака-1).

| Переменная (common.sh) | Путь | Что там | Диск |
|---|---|---|---|
| `PACK_DIR` | `/mnt/dump3/yandex_disk_linux_baby_zergling/Общее/Фотки/МТС/Готовое/пак-2 v.1 (1977-01 - 1977-06) (300 dpi)` | Оригиналы TIFF | dump3, Я.Диск |
| `PACK_NAME` | `пак-2` | Имя пака в базе, проекта CVAT и подпапки share; задаётся явно, иначе имя папки целиком | — |
| `PACK_WORK` | `$MARKUP_ROOT/pack2` | Рабочие файлы пака-2 своей подпапкой, чтобы не мешаться с плоской раскладкой пака-1 | SSD |
| `DB`, `DB_REVIEWED` | `$PACK_WORK/pack2.sqlite`, `pack2_reviewed.sqlite` | Та же схема, что у пака-1 | SSD |
| `DEBUG_DIR` | `$PACK_WORK/debug` | Оверлеи детекторов | SSD |
| `SHARE_ROOT` | `$MARKUP_ROOT/cvat_share` | Общий с паком-1; превью ложатся в `пак-2/1977/{выпуск}/`, проект CVAT отдельный | SSD |
| `LAYOUT_CACHE_DIR` | `/mnt/system/raw/mts/pack2_page_layout` | Кэш surya layout (`scan/` перенесён из `pack2_layout_surya`, 591 полоса); `detect` пополняет сам | SYSTEM |
| `CLEAN_ROOT` | `/mnt/system/raw/mts/pack2_background_blurred` | Очистка: `blurred/`, `debug/`, `report.csv`; ~5 ГиБ | SYSTEM |

Заострённых копий, PDF и внешнего OCR у пака-2 пока нет — переменные появятся в `common.sh` по мере надобности.
