...
| `assemble_pack.sh [год/выпуск]` | `assemble --in-dir --cache-dir` | `out/<год>/<номер>/*.json`, `SHARPENED_DIR` (полоски строк) | `out/<год>/<номер>.md`, `.pages.json`, `cache/…/_boundaries` | Пересборка целиковых md — после правки правил сборки или `.json` полос. К модели — только сомнительные стыки, один запрос на выпуск (~0.05 ¢). ~5 мин на пак. |
