...
|---|---|---|---|---|
| `run_issue_sharpened.sh [год/выпуск]` | `run --only-year --only-issue` | `SHARPENED_DIR/<выпуск>`, `DB_REVIEWED` | `EXTERNAL_OCR_SERVICES_ROOT/{pages,debug,cache}/<выпуск>`, `pages/<год>/<год>_<номер>.pages.json`, `out/<год>/<год>_<номер>.md` | Два этапа: полосы с тегами «Оглавление»/«Годовой указатель» → `toc.json` → остальные полосы со списком статей в промпте. Оглавление вне базы перераспознаётся само (`--on-missed-toc redo` по умолчанию). В конце выпуск собирается в один markdown (переносы и оборванные абзацы через границу полос сшиты; сомнительные стыки — одним запросом модели на выпуск; `#` и `<rubric>` сверены с `toc.json`: одна статья — один `#` с `<!-- article A3 -->`, фантомы продолжений сняты, номера ненумерованных полос выведены по соседям — итог в `.pages.json`; проверка по всем выпускам — `scripts/check_issue_headings.py --pages-dir pages --issues-dir out`). Кэш запросов в `cache/`: повтор с теми же промптами и тайлами в сеть не ходит. ~10 ¢ на выпуск. |
| `run_pack_sharpened.sh` | `run --skip-done` | весь `SHARPENED_DIR`, `DB_REVIEWED` | те же `pages/`, `out/`, `debug/`, `cache/`, `pages/missed_toc.txt` | Идемпотентен, в фон через `setsid … < /dev/null`. Оглавления вне базы перераспознаются сами и видны в логе: проставить по ним теги в CVAT (в т. ч. вето «Не оглавление») и `run_3_from_cvat.sh`. ~$12 на пак. |
| `assemble_pack.sh [год/выпуск]` | `assemble --pages-dir --issues-dir --in-dir --cache-dir` | `pages/<год>/<номер>/*.json`, `SHARPENED_DIR` (полоски строк) | `out/<год>/<год>_<номер>.md`, `pages/<год>/<год>_<номер>.pages.json`, `cache/…/_boundaries`, `cache/…/_headings` | Пересборка целиковых md — после правки правил сборки или `.json` полос. К модели — только сомнительные стыки и (с `--check-headings`) статьи без заголовка, по запросу на выпуск (~0.05 ¢). ~5 мин на пак. В `out/` лежат только md выпусков; sidecar, полосы и `toc.json` — в `pages/`. |
...
- **FineReader на win10 VM** — пакетное распознание через Hot Folder по папке (поэтому промежуточные PDF собираются по паку, а не по годам). Два задания по одним полным промежуточным PDF: бинаризация **с** коррекцией геометрии (`GEO_PDF_DIR`) и **без** неё (`NOGEO_PDF_DIR`, геометрия скана — сюда возвращаются иллюстрации); финальный PDF собирает `final_pdfs` постранично. `pages_with_pics_only` больше не распознаются. «Исправлять ориентацию страницы» валит прогон на боковых иллюстрациях (см. ветку orientation). Имена файлов те же, что у промежуточных.
...
- Регистр `/mnt/system` значим: с 2026-09-20 том смонтирован как /mnt/system строчными; прежний /mnt/SYSTEM заглавными больше не существует.
...
- `final_pdfs` отказывается собирать выпуск, у которого числа страниц двух PDF и число полос в базе расходятся, или образ страницы без коррекции не равен «полоса + поля» — сборка со сдвигом на страницу хуже её отсутствия. В `pack1_reviewed.sqlite` после `from-cvat` номера страниц и поля выпусков пусты — сборщик на них не полагается.
- FineReader кладёт фото отдельными JBIG2-образами (60–120 КБ), а основной образ под ними пуст: сборщик снимает такие образы под иллюстрациями вместо перекодирования JBIG2 (G4 дал бы +40 % размера).
