...
# они нужны раньше, поэтому переносятся как есть, с source=auto, — только виды table,
# line_art_schema и rotated_text (разбор page_layout), ручной растр в целевой базе не
# трогается. Следующий from-cvat заменит их уточнёнными.
...
uv run python -m ocr_utils.scan_markup copy-regions \
    ...
    --out-db "$DB_REVIEWED" \
    --pack-name "$PACK_NAME" \
    --kinds table,line_art_schema,rotated_text
