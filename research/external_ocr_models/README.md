...
`rubric`, `title`, `authors` (`[{name, position}]`), `content_markdown`, `notes`. Разметка
тела: `#` заголовок статьи, `##` подзаголовок, `<rubric>*рубрика*</rubric>`,
`<author>**И. Фетисов**</author>`, `<position>*должность*</position>` (теги — чтобы
отличать от прочего жирного/курсива и от иерархии заголовков; в просмотрщиках невидимы),
таблицы всегда HTML `<table>` (rowspan/colspan, строка на печатную строку),
`> [блок-схема]`, `> [картинка: …]`, сноски `[^1]`.
...
* Промпты — Jinja-шаблоны в `prompts/`; при правке поднимать `PROMPT_VERSION`. Системный
  промпт говорит про советскую и постсоветскую экономическую прессу вообще (журналы и
  газеты); конкретное издание — `--source "журнал «Материально-техническое снабжение», Москва, 1966"`.
