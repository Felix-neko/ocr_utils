...
* `--restore` просит достраивать повреждённые буквы по контексту и помечать их
  `<restored>…</restored>` (плюс список слов в поле `restored`); работает только вместе с
  `--hint "…"` — подсказкой, что и где на полосе повреждено (без неё модели достраивают
  молча, см. отчёт, раздел 8).
