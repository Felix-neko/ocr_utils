...
class RunOptions:
    ...
    write_md: bool = True
    restore: bool = False  # достраивать повреждённые буквы и помечать их <restored>
...
def build_payload(
    spec: ModelSpec, images: list[PreparedImage], options: RunOptions, json_mode: JsonMode, skip_reasoning: bool = False
) -> dict:
    content: list[dict[str, Any]] = [{"type": "text", "text": user_prompt(len(images), options.hint)}]
    for image in images:
    ...
    payload: dict[str, Any] = {
        ...
        "messages": [
            {"role": "system", "content": system_prompt(options.output_mode, options.restore)},
    ...
    if options.output_mode == "json":
        if json_mode == "json_schema":
            payload["response_format"] = {
                "type": "json_schema",
                "json_schema": {"name": "page_transcription", "strict": True, "schema": json_schema(options.restore)},
            }
...
def recognise_page(
    ...
    for json_mode in _fallback_chain(spec, options):
        payload = build_payload(spec, images, options, json_mode, skip_reasoning)
        try:
            ...
            if error.status in FALLBACK_STATUSES and "reasoning" in payload and "reasoning" in error.body.lower():
                ...
                skip_reasoning = True
                try:
                    response = client.chat(build_payload(spec, images, options, json_mode, True))
