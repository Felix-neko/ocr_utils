"""Строки по тем же глифам: наклон по проекции, форма заголовка — и выигрыш только там, где он виден.
...
* **форма заголовка — относительными мерами по остатку сдвигов кусков** (решение пользователя
  2026-09-22): из сдвигов кусков глифов вычитается прямая (общий перенос и доворот строки), и
  остаётся то, что коррекция сделала со строкой сверх них. ``line_bend_ratio`` — усечённый размах
  остатка (p10–p90) к длине строки (дуга заголовка 1973/10 с.32 — 5.2·10⁻³), ``line_step_ratio`` —
  наибольший сдвиг уровня (медианы половин, не меньше двух кусков с каждой стороны) к высоте букв
  (ступенька «Севера» 1968/09 с.52), ``line_wedge_ratio`` — линейный тренд масштаба кусков (клин),
  ``line_stretch_ratio`` — размах масштабов p10–p90 (неравномерное растяжение). У строки, которая
  стала ПРЯМЕЕ (размах остатка базовой по центроидам краски упал на ``STRAIGHTENED_BEND_RATIO``) или
  РОВНЕЕ (|наклон| упал на ``STRAIGHTENED_TILT_DEG``), форма не считается порчей вовсе, а клин
  режется до ``WEDGE_FORGIVEN_MAX``: остаток сдвигов знака не имеет, и убранная дуга даёт тот же
  размах, что и наведённая (1968/01 с.96 — заголовок рубрики выпрямлен).
"""
