...
DEFAULT_THRESHOLDS: dict[str, tuple[float, str]] = {
    ...
    # страницам порчи и двум не-порчи. Ступенька к высоте: порча 0.03–0.08, не порча < 0; клин и
    # неравномерность к высоте: порча 0.05–0.12 (при выпрямлении режутся до 0.04 — 1968/02 с.92).
    # Только заголовки. Дуга к длине откалибрована по паку на мере из остатка сдвигов (после гашения
    # у выпрямленных строк дуга ненулевая на 77 страницах пака): страницы порчи пользователя лежат на
    # 5.2–9.4·10⁻³, при пороге 5·10⁻³ флаг получают 40 страниц, при 3·10⁻³ — 75 (пояс 3–5·10⁻³ — дуги
    # 0.25–0.4 мм у невыпрямленных заголовков, кандидат на следующий просмотр).
    "line_bend_ratio": (5.0e-3, "bend_ratio"),
    ...
    # «равнение» уход 0.65–5 мм. Порог поднят с 0.8 до 1.0 после правки меры (2026-09-23): на
    # исправленной шкале четырнадцать страниц «равнение перекосило», отобранных пользователем
    # глазами, дают 1.06–3.45 мм, а законная правка 1968/02 с.69 (эталон good, подтверждён
    # пользователем) — 0.8 мм; при 1.0 мм флаг получают 1634 страницы пака, при 0.8 — 1839,
    # при 1.2 теряется слабейшая страница пользователя. Балл — от кромки в мм, чтобы против
    # выигрыша (в мм и градусах) стоял уход, а не угол: снятая трапеция 1975/05 с.61 даёт сдвиг
    # по полю 2.3° при уходе 1.5 мм.
    "edge_shear_delta_mm": (1.0, "shear"),
}
