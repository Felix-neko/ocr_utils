...
* **Потрачено** на всё исследование (пробник на 11 прогонах, повторы, выпуск целиком на
  четырёх моделях, повреждённые сканы, повтор выпуска на DeepSeek с промптом v12) — **$1.93**
  из $30.
...
|---|---|---|
| полос / сбоев | 97 / 0 | 97 / 0 |
| стоимость по `usage.cost` | $0.094 (0,097 ¢/полоса) | **$0.0916 (0,094 ¢/полоса**, медиана та же, максимум 0,136 ¢) |
| токены вход / выход | — | 330 071 / 104 725 |
| провайдер, латентность | DeepSeek, 7 с | DeepSeek (все 97), 7 с; 4 потока — ~4 минуты на выпуск |
| CER к FineReader (медиана / текст) | 0,003 / 0,031 | 0,003 / 0,028 |
| таблицы | 3 HTML + 8 GFM | **11 HTML, 0 GFM** |
| боковые шапки найдены | 16/17 | 16/17 |
| номер страницы верно | 83/96 | 82/96 |
| `#`/`##`/`###` | 37/16/19 | 45/21/10 |

Сверка по балансу OpenRouter: до прогона потрачено $1.8458, после — $1.9250 (разница
$0.079 — баланс обновляется с задержкой; сумма по ответам $0.0916 точнее). Экстраполяция
на пак из 12 135 полос — ≈ $11,5. Всего на исследование к этому моменту — $1.93 из $30.
