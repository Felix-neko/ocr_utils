...
* Имена функций, методов, классов и переменных — на английском в американском написании: `recognize`, `normalize`, `summarize`, `serialize`, `modernize`, `color` (не `recognise`, `colour`). Это про идентификаторы; текст промптов и прозы правило не трогает.
