...
| `curved_layout` | Разбор текста страницы с кривыми строками: локальные межколонники и зоны вёрстки, осевые кривые строк, гладкие огибающие текстовых блоков, выключка блока (лево/право/формат). Стенд, в детектор геометрии не подключён | `python -m ocr_utils.curved_layout analyze` |
