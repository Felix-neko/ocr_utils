...
def blend_roi(orig: np.ndarray, filled: np.ndarray, mask: np.ndarray, feather: int) -> np.ndarray:
    ...
    На сохранности закрашиваемого объекта спад внутрь не сказывается: маску перед
    закраской дилатируют на десятки пикселей (``FINGER_DILATE_PX`` у пальцев,
    ``MASK_DILATE_PX`` у разметки из CVAT), и её наружные ``feather`` пикселей — это
    уже фон вокруг объекта, а не сам объект.
