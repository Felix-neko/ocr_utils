...
class PageDewarpEngine(DewarpEngine):
    """Расширенный вариант: поля 0 (ничего не режется по бокам), ширина ровно как у входа,
    цветной ремап; размер копии для детекции строк крупнее библиотечного."""

    name = "pagedewarp"

    def load(self, device: str) -> None:
        self.no_binary = True  # нейросеть не нужна; сырой grayscale/цвет, без порога
    ...
    def dewarp(self, img_bgr: np.ndarray) -> Optional[np.ndarray]:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            if not wimg.written:
                ...
                return None
            return _remap_color(wimg.cv2_img, wimg.page_dims, wimg.params, cfg)
