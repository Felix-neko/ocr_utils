...
# Веса DocShadow лежат рядом с прочими нейромоделями проекта
DOCSHADOW_DIR = Path(__file__).resolve().parents[2] / "finger_models" / "docshadow"
_DOCSHADOW_WEIGHTS = {"sd7k": "SD7K.pth", "kligler": "Kligler.pth", "jung": "Jung.pth"}
...
# ============================================================
# Нейросетевой метод (DocShadow / FSENet)
# ============================================================


def _load_docshadow(variant: str, device: str):
    """Ленивая загрузка DocShadow с нужными весами (variant: sd7k/kligler/jung)."""
    key = f"docshadow:{variant}"
    if key not in _NET_CACHE:
        from .docshadow_net import Model

        weight_path = DOCSHADOW_DIR / _DOCSHADOW_WEIGHTS[variant]
        model = Model().to(device).eval()
        ckpt = torch.load(weight_path, map_location="cpu")
        sd = ckpt.get("state_dict", ckpt)
        sd = {(k[7:] if k.startswith("module") else k): v for k, v in sd.items()}
        model.load_state_dict(sd, strict=True)
        _NET_CACHE[key] = model
    return _NET_CACHE[key]


def _docshadow(bgr: np.ndarray, variant: str, device: str, max_side: int) -> np.ndarray:
    """Прогоняет DocShadow по всему кадру (сеть обучена убирать тень документа) и
    возвращает скорректированный BGR. Инференс на уменьшенной до ``max_side`` копии
    (сеть частотно-осведомлённая, детали восстанавливаются пирамидой), затем апскейл."""
    net = _load_docshadow(variant, device)
    rgb = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
    H, W = rgb.shape[:2]
    s = min(1.0, max_side / max(H, W))
    w2, h2 = (int(W * s) // 4 * 4, int(H * s) // 4 * 4)  # кратно 4 для пирамиды
    small = cv2.resize(rgb, (w2, h2), interpolation=cv2.INTER_AREA)
    t = torch.tensor(small, dtype=torch.float32).permute(2, 0, 1).unsqueeze(0).to(device) / 255.0
    with torch.no_grad():
...
def correct_finger_shadow(
    ...
    if method == "classic":
        ...
        corrected = _docshadow(bgr, method.split("-", 1)[1], device, max_side)
    else:
