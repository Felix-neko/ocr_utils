...
from ocr_utils.legacy.config import OCR_LANGUAGE, OCR_UPSCALE_RATIO
...
def prepare_images_for_ocr(input_pdf: Path, output_pdf: Path, upscale_ratio: float = 2.0, deskew: bool = False) -> Path:
    ...
    logger.info(
        "Подготовка изображений для OCR: %s → %s (upscale_ratio=%.2f, deskew=%s)",
        input_pdf,
        output_pdf,
        upscale_ratio,
        deskew,
...
def run_ocr(
    ...
    try:
        ...
        logger.info(
            "Этап 2: OCR через ocrmypdf (clean=%s, rotate=%s, jobs=%d) → %s", clean, rotate_pages, jobs, tmp_ocr_path
        )
        ocrmypdf.ocr(
