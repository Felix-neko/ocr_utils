...
Запуск (см. README.md этого подпакета):
    uv run python -m ocr_utils.legacy.defocus_detection "/путь/к/выпуску"          # режим по умолчанию
    uv run python -m ocr_utils.legacy.defocus_detection "/путь" --quiet            # только подозрительные
...
from ocr_utils.legacy.defocus_detection.image_io import collect_inputs, read_image_gray, save_heatmap
from ocr_utils.legacy.defocus_detection.pipeline import VERDICTS, analyze, verdict
