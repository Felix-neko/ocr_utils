...
Примеры:
    ...
    uv run python -m ocr_utils.legacy.finger_removal IN OUT --mask-only --debug-dir OUT/debug

    # удалить пальцы через LaMa
    uv run python -m ocr_utils.legacy.finger_removal IN OUT --inpaint-method lama

    # через Stable Diffusion с собственным промптом
    uv run python -m ocr_utils.legacy.finger_removal IN OUT --inpaint-method sd \\
...
from ocr_utils.legacy.finger_removal.inpainting import DEFAULT_SD_NEGATIVE, DEFAULT_SD_PROMPT, inpaint_image
from ocr_utils.legacy.finger_removal.masking_batch import build_finger_mask_batch, overlay_mask
...
@click.option("--conf", default=0.01, show_default=True, help="Порог уверенности YOLO (ниже = больше детекций)")
def main(
    input_dir: Path,
    output_dir: Path,
    mask_method: str,
        ...
        try:
            ...
            if auto_rgbs:
                auto_results = build_finger_mask_batch(
                    auto_rgbs, models, method=mask_method, edge_frac=edge_frac, dilate_px=dilate_px, conf=conf
                )
            else:
                auto_results = []
