...
Примеры:
    ...
    uv run python -m ocr_utils.legacy.dewarp --input-dir IN --method docscanner --limit 10

    # все движки последовательно, каждый в свою папку
    uv run python -m ocr_utils.legacy.dewarp --input-dir IN --method all
"""
...
from ocr_utils.legacy.dewarp.engines import ENGINES, get_engine
...
def run_pagedewarp_parallel(files: list[Path], out_dir: Path, no_binary: bool, jobs: int, extended: bool) -> None:
    ...
    from ocr_utils.legacy.dewarp.engines.pagedewarp import pagedewarp_one
...
@click.option(
    "--jobs", default=4, show_default=True, help="page-dewarp: число процессов (по умолчанию 4; 0 — все ядра CPU)"
)
@click.option(
