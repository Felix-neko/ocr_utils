...
from ocr_utils.legacy.dewarp.engines.download import MODELS_DIR, add_to_path, ensure_repo, forget_modules
...
class DewarpNetEngine(DewarpEngine):
    ...
    def load(self, device: str) -> None:
        ...
        if not (wc_path.exists() and bm_path.exists()):
            raise FileNotFoundError(f"Нет весов DewarpNet в {weights_dir} (нужны {WC_NAME} и {BM_NAME}).")
