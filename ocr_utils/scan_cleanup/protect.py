...
# Доля кадра, ниже которой область анализа считается вырожденной и пороги считаются
# по всему кадру (см. :func:`analysis_roi`). Числу здесь быть точным незачем: доля
# либо нормальная (у полосы с иллюстрацией остаётся больше половины кадра), либо
# нулевая — промежуточных случаев в паке нет.
MIN_ANALYSIS_FRAC = 0.05
...
def analysis_roi(shape: "tuple[int, int]", markup: PageMarkup) -> "np.ndarray | None":
    ...
    ЕСЛИ ОБЛАСТИ АНАЛИЗА ПОЧТИ НЕ ОСТАЛОСЬ — тоже ``None``. Так бывает, когда
    полосу целиком накрывает ОДИН ``color_text``: защищать его нельзя (это буквы,
    и сплошной прямоугольник оставил бы неразмытым весь фон между ними), а вычесть
    из области анализа — значит не оставить ничего. Полоса тогда молча уходила в
    копию: ``has_content`` на пустой выборке отвечает «контента нет». По паку-1
    таких полос 13 (например, 1976/12/0020_1L). Правильнее считать пороги по всему
    кадру: цветной набор для Оцу — та же краска, что и чёрный.
    """
    if not markup.regions:
        return None
    covered = rects_mask(shape, markup.regions)
    roi = cv2.bitwise_not(covered)
    if np.count_nonzero(roi) < MIN_ANALYSIS_FRAC * roi.size:
        return None
    return roi
