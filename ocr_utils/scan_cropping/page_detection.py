...
получения: отбор боксов и доводку силуэта. Сами сети (YOLO-World, SAM) живут в
``scan_cropping.gpu_models.GpuModels``, дальнейшее — поворот, кроп, заливка фона —
в ``scan_cropping.geometry`` / ``cropping``, которые эту маску потребляют.
"""
...
def detect_page_mask(bgr: np.ndarray, models, frame_area: Optional[float] = None) -> np.ndarray:
    ...
    boxes, confs, is_fabric = models.detect_page_boxes(bgr, conf=CONF)
    if len(boxes) == 0:
    ...
    for m_bin in models.segment_boxes(bgr, sam_boxes):
        if MIN_PAGE_FRAC * frame_area <= m_bin.sum() <= MAX_PAGE_FRAC * frame_area:
            mask = cv2.bitwise_or(mask, m_bin.astype(np.uint8) * 255)
    return mask
