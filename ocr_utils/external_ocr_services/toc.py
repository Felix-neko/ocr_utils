...
def _is_toc_entry_line(line: str) -> bool:
    """Строка — элемент списка оглавления: пункт с «— где», пункт без него или рубрика ``<rubric-in-toc>``.
...
def ensure_toc_block(body: str) -> tuple[str, bool]:
    ...
    элемент оглавления (пункт «Автор. Название — страница» или ``<rubric-in-toc>``); всё между
...
def render_toc_block(page: TocPage) -> str:
    ...
    Returns:
        Текст блока: рубрики в ``<rubric-in-toc>``, статьи пунктами «Автор. Название — страница»
...
class ParsedEntry:
    ...
    Args:
        ...
        rubric: Рубрика контекста — последняя ``<rubric-in-toc>`` выше пункта; ``None`` до первой.
