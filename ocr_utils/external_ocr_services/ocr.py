...
# Версии промпта, чьи готовые полосы считаются сделанными для --skip-done наравне с текущей: разбор
# приводит их выход к текущему виду (v20 → v21: сноски `[^1]` → надстрочные цифры при чтении .json),
# и перегонять пак ради них незачем. При смене смысла разметки список сужать до текущей версии.
COMPATIBLE_PROMPT_VERSIONS = frozenset({20, PROMPT_VERSION})
...
def is_done(pages_dir: Path, job: PageJob) -> bool:
    ...
    # Исключение — версии, чей выход разбор приводит к текущему виду (COMPATIBLE_PROMPT_VERSIONS).
    if meta.get("prompt_version") not in COMPATIBLE_PROMPT_VERSIONS:
        return False
