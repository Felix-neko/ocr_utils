...
# Между двумя рядами букв внутри половины тег обязателен (``_TAGS``, «+»), иначе получается
# вложенный квантификатор ``[буквы]+(?:[буквы]+)*`` с экспоненциальным перебором на длинном слове
# без дефиса: «Красноярскинструментподшипникснабсбыт» (37 букв) вешал прогон 1976/12 на 2³⁵ разбиений.
_TAG = r"(?:</?(?:supplied|unclear)>)*"
_TAGS = r"(?:</?(?:supplied|unclear)>)+"
_HALF_A = rf"{_TAG}[а-яёА-ЯЁ]{{2,}}(?:{_TAGS}[а-яёА-ЯЁ]+)*{_TAG}"
_HALF_B = rf"{_TAG}[а-яё]{{2,}}(?:{_TAGS}[а-яё]+)*{_TAG}"
HYPHENATED = re.compile(rf"(?<![\w-])(?P<a>{_HALF_A})-\n?(?P<b>{_HALF_B})(?![\w-])")
