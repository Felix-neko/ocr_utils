...
DEFAULT_THRESHOLDS: dict[str, tuple[float, str]] = {
    ...
    # мм; кромки фотографий (``raster.py``): рост сагитты кромки A − B и рост ухода конца
    # кромки от оси. Целые снимки: 1970/12 с.76 — 0.04 / 0.5, 1973/03 с.58 — 0.12 / 0.17;
    # перекошенные: 1970/10 с.71 — 3.9 / 11.9, 1971/04 с.44 — 1.5 / 13.2, 1971/07 с.43 (волна
    # верхней кромки) — 0.65 / 2.25. Доля тайлов снимка без пары порчу не отделяет: у целого
    # снимка их и так до 50 % (растровая сетка), у перекошенного 55–75 %.
    "raster_edge_bend_mm": (0.8, "photo_bend"),
    "raster_edge_tilt_mm": (1.5, "photo_tilt"),
...
# (СРЕДНИЙ наклон горизонтальных штрихов при ≥ 2 чертах — все черты разом), погнутый line art
# и перекошенная фотография.
...
UNFORGIVABLE = ("hstroke_tilt_wmean_delta", "field_lineart_weak_frac", "raster_edge_bend_mm", "raster_edge_tilt_mm")
